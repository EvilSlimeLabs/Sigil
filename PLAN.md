...
- Placeable on the **top of a block** (lying flat on the floor) and on **all four walls** (hanging vertically). **Not on a ceiling**: `minecraft:placement_filter` allows `["up", "side"]` and omits `down` — and because maps were still going up on ceilings in a live world despite it, the custom component's `beforeOnPlayerPlace` cancels a downward face outright. The filter is the declarative statement of intent; the handler is what enforces it.
